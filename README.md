...
### v0.9.23.0
- **パフォーマンス大幅向上**: ネットワーク情報の取得処理において、重い PowerShell スクリプトへの依存を最小限に抑え、C# ネイティブ API (`System.Net.NetworkInformation`) とのハイブリッド方式に刷新しました。
- **堅牢性の向上 (スレッドセーフ化)**: Wi-Fi の切断・接続時などにOSから連続して発行されるネットワーク変更イベントに対し、固定の待機時間（Task.Delay）に依存しない、堅牢なフラグ制御ベースのステートマシンと排他制御 (`SemaphoreSlim`) を導入。UIコンボボックスのアダプタ増殖バグを完全に排除しました。
- **多言語対応 (I18n) の完全化**: 一部残存していたハードコードされた日本語文字列を完全に排除し、全テキストをリソースファイル (`Resources.resx`, `Resources.ja-JP.resx`) による管理へ移行。英語環境と日本語環境での適切な表示を保証します。
